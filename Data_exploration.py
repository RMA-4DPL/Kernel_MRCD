...
if len(corrected_data.shape)>3:
    corrected_data = corrected_data.squeeze()
...
raw_bgr_idx = band_indices(BGR, raw_wavelengths, data.shape[2])
raw_above_idx = band_indices(above_r_wavelengths, raw_wavelengths, data.shape[2])
...
for row in range(1, 6):
    ...
    axes[row, 0].imshow(normalize(data[:, :, raw_band]), cmap='gray')
...
plt.tight_layout()
os.makedirs(os.path.join(figure_filepath, args.dataset), exist_ok=True)
fig.savefig(os.path.join(figure_filepath, args.dataset,'bgr_and_bands.png'), dpi=150)
plt.show()

spectral_wavelengths = wavelengths[:corrected_data.shape[2]]
class_ids = [c for c in sorted(labels_ids.keys())]

fig2, ax2 = plt.subplots(figsize=(12, 7))
colors = plt.cm.tab20(np.linspace(0, 1, len(class_ids)))
for color, class_id in zip(colors, class_ids):
    spectra = corrected_data[labels == class_id]
